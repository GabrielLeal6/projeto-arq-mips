...
loop_ler: # entrada do loop de leitura do teclado
	lw $t1, 0($t0) # carrega o status do teclado
	andi $t1, $t1, 1 # Isola o bit "pronto"
	beqz $t1, loop_ler # Se não estiver pronto, volta para o começo do loop

	lw $t2, 4($t0) # carrega o caractere pronto em $t2

loop_display: # entrada da parte de display
	lw $t3, 8($t0) # carrega o status do display
	andi $t3, $t3, 1 # Isola o bit "pronto"
	beqz $t3, loop_display # Se não estiver pronto, volta para o começo do loop
	...
	j loop_ler # se não foi enter, continua o loop
...
fim_read_line: # saída do MMIO para 
	...
	jr $ra # retorna para a parte do código para onde foi invocado